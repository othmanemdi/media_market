...
$status_commande = $pdo->query("SELECT * FROM status_commande ORDER BY id DESC;")->fetchAll();
...
if (isset($_POST['role_add'])) {
    ...
    $req = $pdo->prepare('SELECT id FROM status_commande WHERE nom = ? LIMIT 1');
    $req->execute([$nom]);
    $maruqe = $req->fetch();
    ...
    if (empty($errors)) {
        $user = $pdo->prepare("INSERT INTO status_commande SET 
                nom = :nom
        ");
        $user->execute(
            [
        ...
        header('Location: status_commande');
        die();
    }
}
...
if (isset($_POST['role_update'])) {
    ...
    if (empty($errors)) {
        $user = $pdo->prepare("UPDATE status_commande SET 
                nom = :nom WHERE id = :id
        ");
        $user->execute(
            [
...
if (isset($_POST['role_delete'])) {
    ...
    $id = (int)$_POST['role_id'];
    if ($id > 3) {
        $pdo->query("DELETE FROM status_commande WHERE id = $id");
...
<h3>Status de commande</h3>


<div class="card shadow-sm">
    <div class="card-header">
        <h4>Liste des status de commande</h4>
    </div>
    ...
    <div class="card-body">
        ...
        <div class="table-responsive">
            ...
            <table class="table table-bordered table-hover table-sm table-striped">
                ...
                <tbody>
                    <?php foreach ($status_commande as $key => $s) : ?>
                        <tr>
                            <th>
                                <?= $s->id ?>
                            </th>
                            ...
                            <td>
                                ...
                                <button type="button" class="btn btn-dark btn-sm" data-bs-toggle="modal" data-bs-target="#role_update_<?= $s->id ?>">
                                    <i class="fas fa-wrench"></i>
                                    Modifier
                                </button>

                                <div class="modal fade" id="role_update_<?= $s->id ?>" tabindex="-1" aria-labelledby="role_update_<?= $s->id ?>Label" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                                <h6 class="modal-title" id="role_update_<?= $s->id ?>Label">
                                                    <i class="fas fa-wrench"></i>
                                            ...
                                            <form method="post">
                                                <div class="modal-body">
                                                    ...
                                                    <div class="mb-3">
                                                        ...
                                                        <input name="nom" type="text" class="form-control" id="nom" value="<?= strtoupper($s->nom) ?>" placeholder="Client">
                                                ...
                                                <div class="modal-footer">
                                                    ...
                                                    <input type="hidden" name="role_id" value="<?= $s->id ?>">
                                ...
                                <button type="button" class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#role_delete_<?= $s->id ?>">
                                    <i class="fas fa-trash-alt"></i>
                                    Supprimer
                                </button>

                                <div class="modal fade" id="role_delete_<?= $s->id ?>" tabindex="-1" aria-labelledby="label_<?= $s->id ?>" aria-hidden="true">
                                    <div class="modal-dialog">
                                        <div class="modal-content">
                                            <div class="modal-header">
